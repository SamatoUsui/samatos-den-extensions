package eu.kanade.tachiyomi.extension.es.samatoden
...
class SamatoDenMangaFactory : SourceFactory {
    override fun createSources(): List<Source> = listOf(SamatoDenManga())
}
...
class SamatoDenManga : HttpSource() {
    ...
    override fun searchMangaRequest(page: Int, query: String, filters: FilterList): Request =
        GET(feedUrl(page, query.trim()), headers)
    ...
    private fun buildSequencePages(base: String, start: Int, end: Int, extension: String): List<String> =
        (start..end).map { "$base$it$extension" }

    private fun inferExtensionFromBase(base: String): String =
        when {
    ...
    private fun extractChapterTitles(html: String): Map<String, String> =
        OPTION_TITLE_REGEX.findAll(html).associate { match ->
